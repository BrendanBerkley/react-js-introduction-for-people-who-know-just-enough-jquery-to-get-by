...
There seems to be an issue with this page, where (1) the browser constantly reloads itself while viewing this page, and (2) as a result, your browser's back button gets broken. There seems to be an issue in the JSBin code I'm using, and [I reported the issue here](https://github.com/jsbin/jsbin/issues/2464). My apologies.
...
Unless you already have a JSBin account, <strong>head to <a href="http://jsbin.com/" target="_blank">jsbin.com</a> to create an account.</strong> Click **Login or Register** on the menu to create an account.
...
Now we need to "attach" this UI to the DOM in order to see `Hello World`. To do this, add `React.render()`**:
...
`React.render` takes two arguments. The first argument is the UI object, which is `<VariableName />`. The second argument is the DOM object (in this case, `document.body`). The above code puts the `TweetBox` UI object inside `body` and displays it.
...
**Next, we'll modify the event handler** to set the state's `text` field to whatever is currently in the text box. To do this, **we use a special in method called `setState` and pass the updated key-value pair.**
...
In other words, the button was disabled if the text's length was 0. **Now, the button is disabled if**:
...
Or, you can simplify the above code by utilizing `remainingCharacters()`. If there are 140 characters remaining, that means no text is entered, so the button should be disabled.
